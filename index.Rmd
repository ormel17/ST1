...
data_ts <- ts(data$meantemp, frequency = 12)  
...
data$meantemp_diff <- c(NA, diff(data$meantemp_log))  
...
lines(data$date, data$meantemp, col = "red")  
...
# Ajuste del formato de la fecha y creación del DataFrame requerido para Prophet
data <- data %>%
...
input_test <- create_sequences(test_data$scaled_log_meantemp, n_steps)

# Función para entrenar, predecir y evaluar modelos
...
### Análisis de los Gráficos de la Red Elman

* **Gráfico del Error Ponderado (SSE) a lo Largo de las Iteraciones:**

**Descripción:**
Este gráfico muestra cómo se reduce el error ponderado (SSE, Sum of Squared Errors) durante las iteraciones de entrenamiento de la red Elman.
...
El gráfico muestra que la red Elman es capaz de minimizar el error de manera efectiva durante el entrenamiento.

* **Gráfico de Predicción con Red Elman:**

**Descripción:**
Este gráfico compara las predicciones de la red Elman (línea azul) con los valores reales de la temperatura media logarítmica (línea roja) durante el período de prueba.
...
La capacidad de la red Elman para capturar la dinámica temporal es evidente en la similitud entre las líneas de predicción y los valores reales.

* **Gráfico de Errores de Predicción - Elman:**

**Descripción:**
Este gráfico muestra los errores de predicción (diferencia entre las predicciones y los valores reales) a lo largo del período de prueba.
...
La línea roja discontinua en el cero ayuda a visualizar la distribución de los errores, mostrando que los errores positivos y negativos se compensan entre sí.


## Red Neuronal Jordan
```{r}

jordan_model <- train_predict_evaluate("jordan", input_train, output_train, input_test)

```



### Análisis de los Gráficos de la Red Jordan

* **Gráfico del Error Ponderado (SSE) a lo Largo de las Iteraciones:**

**Descripción:**
Similar al gráfico de la red Elman, este gráfico muestra cómo se reduce el error ponderado durante las iteraciones de entrenamiento de la red Jordan.
...
El modelo es eficaz en minimizar el error durante el entrenamiento, similar a la red Elman.

* **Gráfico de Predicción con Red Jordan:**

**Descripción:**
Este gráfico compara las predicciones de la red Jordan (línea azul) con los valores reales de la temperatura media logarítmica (línea roja) durante el período de prueba.
...
La red Jordan es igualmente competente en capturar la dinámica temporal de los datos.

* **Gráfico de Errores de Predicción - Jordan:**

**Descripción:**
Este gráfico muestra los errores de predicción de la red Jordan a lo largo del período de prueba.
...
La distribución de errores alrededor de la línea roja discontinua (cero) indica que no hay un sesgo sistemático en las predicciones.

### Comparación de Redes Elman y Jordan:

**Convergencia y Entrenamiento:**

Ambas redes muestran una rápida disminución del error durante las primeras iteraciones y convergen alrededor de la iteración 200.
Ambas redes minimizan el error de manera efectiva durante el entrenamiento.

**Precisión de Predicción:**

Tanto la red Elman como la red Jordan capturan bien la tendencia y la estacionalidad de los datos.
Las predicciones de ambas redes siguen de cerca los valores reales, con ligeras desviaciones en los picos y valles.

**Errores de Predicción:**

Los errores de predicción de ambas redes se distribuyen alrededor de cero, sin sesgo sistemático.
Las mayores desviaciones ocurren al inicio del período de prueba, con una tendencia a estabilizarse.

**Conclusión:**
Las redes Elman y Jordan son eficaces para la predicción de series temporales, mostrando una precisión y un comportamiento de errores similares. La elección entre una y otra puede depender de la especificidad del problema y las características de los datos.
